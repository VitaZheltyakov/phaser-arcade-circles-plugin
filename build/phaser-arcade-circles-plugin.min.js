...
Math.sin(angleCollision)}else if(!body1.isCircle&&!body2.isCircle||!collisionAsCircle){body2.y+=overlap;body2.velocity.y=v1-v2*body2.bounce.y;if(body1.moves)body2.x+=(body1.x-body1.prev.x)*body1.friction.x}return true};Phaser.Physics.Arcade.prototype.distanceBetweenCenters=function(source,target){var dx=source.center.x-target.center.x;var dy=source.center.y-target.center.y;return Math.sqrt(dx*dx+dy*dy)};Phaser.Physics.Arcade.prototype.angleBetweenCenters=function(source,target){var dx=target.center.x-
source.center.x;var dy=target.center.y-source.center.y;return Math.atan2(dy,dx)};Phaser.Physics.Arcade.Body.prototype.checkWorldBounds=function(){var pos=this.position;var bounds=this.game.physics.arcade.bounds;var check=this.game.physics.arcade.checkCollision;if(this.isCircle){var bodyBounds={};bodyBounds.x=this.center.x-this.radius;bodyBounds.y=this.center.y-this.radius;bodyBounds.right=this.center.x+this.radius;bodyBounds.bottom=this.center.y+this.radius;if(bodyBounds.x<bounds.x&&check.left){pos.x=
bounds.x-this.halfWidth+this.radius;this.velocity.x*=-this.bounce.x;this.blocked.left=true}else if(bodyBounds.right>bounds.right&&check.right){pos.x=bounds.right-this.halfWidth-this.radius;this.velocity.x*=-this.bounce.x;this.blocked.right=true}if(bodyBounds.y<bounds.y&&check.up){pos.y=bounds.y-this.halfHeight+this.radius;this.velocity.y*=-this.bounce.y;this.blocked.up=true}else if(bodyBounds.bottom>bounds.bottom&&check.down){pos.y=bounds.bottom-this.halfHeight-this.radius;this.velocity.y*=-this.bounce.y;
this.blocked.down=true}}else{if(pos.x<bounds.x&&check.left){pos.x=bounds.x;this.velocity.x*=-this.bounce.x;this.blocked.left=true}else if(this.right>bounds.right&&check.right){pos.x=bounds.right-this.width;this.velocity.x*=-this.bounce.x;this.blocked.right=true}if(pos.y<bounds.y&&check.up){pos.y=bounds.y;this.velocity.y*=-this.bounce.y;this.blocked.up=true}else if(this.bottom>bounds.bottom&&check.down){pos.y=bounds.bottom-this.height;this.velocity.y*=-this.bounce.y;this.blocked.down=true}}};Phaser.Physics.Arcade.Body.render=
function(context,body,color,filled){if(filled===undefined)filled=true;color=color||"rgba(0,255,0,0.4)";if(filled){context.fillStyle=color;if(body.isCircle){context.beginPath();context.arc(body.center.x-body.game.camera.x,body.center.y-body.game.camera.y,body.radius,0,2*Math.PI,false);context.fill()}else context.fillRect(body.position.x-body.game.camera.x,body.position.y-body.game.camera.y,body.width,body.height)}else{context.strokeStyle=color;if(body.isCircle){context.beginPath();context.arc(body.position.x-
body.game.camera.x+body.radius,body.position.y-body.game.camera.y+body.radius,body.radius,0,2*Math.PI,false);context.stroke()}else context.strokeRect(body.position.x-body.game.camera.x,body.position.y-body.game.camera.y,body.width,body.height)}}})(window,Phaser);